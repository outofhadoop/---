...
undefined, number, boolean, string, null 是原始类型

BigInt, Object, Symbol 是包装类型

| 基本类型      | 对应的包装类型        |
| --------- | -------------- |
| String    | String 对象包装类型  |
| Number    | Number 对象包装类型  |
| Boolean   | Boolean 对象包装类型 |
| Undefined | 无对应包装类型        |
| Null      | 无对应包装类型        |
| Symbol    | Symbol 对象包装类型  |

undefined, number, boolean, string, Object, null 是es5的

Symbol是es6 (es2015)新增

BigInt是es11 (es2020)新增
...
最终，a、b、c分别被赋予了数组第0、1、2个索引位的值： ![img](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/e55fc36b191340e69698782fbd67ef4f~tplv-k3u1fbpfcp-watermark.awebp) 数组里的0、1、2索引位的元素值，精准地被映射到了左侧的第0、1、2个变量里去，这就是数组解构的工作模式。还可以通过给左侧变量数组设置空占位的方式，实现对数组中某几个元素的精准提取：
