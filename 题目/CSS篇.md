...
| ---------------------------- | -------------------------------------------------------- | -------------------------------------------------------- |
| **在渲染树中**                | 元素完全从渲染树中消失，不占据空间                      | 元素仍在渲染树中，占据空间，但内容不可见                  |
| **是否是继承属性**             | 非继承属性，子孙节点消失，无法通过修改属性显示          | 继承属性，子孙节点消失是因为继承了`hidden`，可通过修改为`visible`显示 |
| **文档流影响**                | 修改`display`可能导致文档重排                           | 修改`visibility`只会导致本元素的重绘                      |
| **读屏器表现**                | 不会被读取                                              | 会被读取                                                |
